...
export default function CreateCategoryForCard({ handleClose, card }) {
    ...
    const getTable = (categories, card) => {
        ...
        return (categories[0] ? <Paper sx={{ width: "100%", overflow: "hidden" }}>
            <TableContainer>
                    ...
                    <TableBody>
                        {categories.map((x) => (
                            <TableRow hover role="checkbox" tabIndex={-1} key={x.id}>
                                ...
                                <TableCell>
                                    <Button onClick={() => addCategory(x)} variant="outlined">Add to Card</Button>
                                    <span> </span>
                                </TableCell>
                            </TableRow>
                        ))}
    ...
    return (
        <div className='modalContainerSmall'>
            ...
            <div className="container">
                {getTable(categories, card)}
            </div>

            {showAlert && <Alert severity='success'>Category successfully added</Alert>}
